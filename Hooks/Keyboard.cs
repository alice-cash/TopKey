...
namespace TopKey.Hooks
{
    ...
    public class KeyboardHook : IDisposable
    {
        ...
        private DllImports.HookHandlerDelegate proc;
        ...
        public KeyboardHook()
        {
            proc = new DllImports.HookHandlerDelegate(HookCallback);
            using (Process curProcess = Process.GetCurrentProcess())
            using (ProcessModule curModule = curProcess.MainModule)
            {
                hookID = DllImports.SetWindowsHookEx(WH_KEYBOARD_LL, proc,
                    DllImports.GetModuleHandle(curModule.ModuleName), 0);
            }
        }
        ...
        private void CheckModifiers()
        {
            ...
            if ((DllImports.GetKeyState(VK_CAPITAL) & 0x0001) != 0)
            {
                //CAPSLOCK is ON
                sb.AppendLine("Capslock is enabled.");
            }

            if ((DllImports.GetKeyState(VK_SHIFT) & 0x8000) != 0)
            {
                //SHIFT is pressed
                sb.AppendLine("Shift is pressed.");
            }
            if ((DllImports.GetKeyState(VK_CONTROL) & 0x8000) != 0)
            {
                //CONTROL is pressed
                sb.AppendLine("Control is pressed.");
            }
            if ((DllImports.GetKeyState(VK_MENU) & 0x8000) != 0)
            {
                //ALT is pressed
            ...
            int nCode, IntPtr wParam, ref KBDLLHOOKSTRUCT lParam)
        {
            ...
            if (nCode >= 0)
            {
                if (wParam == (IntPtr)WM_KEYUP || wParam == (IntPtr)WM_SYSKEYUP || wParam == (IntPtr)WM.KEYDOWN || wParam == (IntPtr)WM.SYSKEYDOWN)
                {
                    ...
                    OnKeyIntercepted(new KeyboardHookEventArgs((Keys)lParam.vkCode, AllowKey, (WM)wParam));
                }
            ...
            return DllImports.CallNextHookEx(hookID, nCode, wParam, ref lParam);
        ...
        public class KeyboardHookEventArgs : System.EventArgs
        {
            ...
            public WM WindowMessage
            {
                get { return windowMessage; }
            }
            ...
            public KeyboardHookEventArgs(Keys evtKeyCode, bool evtPassThrough, WM etvWindowMessage)
            {
                keyName = ((Keys)evtKeyCode).ToString();
                keyCode = evtKeyCode;
                passThrough = evtPassThrough;
                windowMessage = etvWindowMessage;
            }
